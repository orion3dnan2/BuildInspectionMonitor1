...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('books.index') }}" class="inline-flex items-center text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">إضافة قيد جديد</h2>
        </div>
        ...
        <form method="POST" action="{{ route('books.store') }}" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="book_number" value="{{ old('book_number', $bookNumber) }}" readonly
                ...
                <div>
                    ...
                    <select name="book_type" required
                        ...
                        <option value="incoming" {{ old('book_type') == 'incoming' ? 'selected' : '' }}>وارد</option>
                        <option value="outgoing" {{ old('book_type') == 'outgoing' ? 'selected' : '' }}>صادر</option>
                        <option value="internal" {{ old('book_type') == 'internal' ? 'selected' : '' }}>داخلي</option>
                        <option value="circular" {{ old('book_type') == 'circular' ? 'selected' : '' }}>تعميم</option>
                        <option value="decision" {{ old('book_type') == 'decision' ? 'selected' : '' }}>قرار</option>
                    </select>
                    @error('book_type')
                ...
                <div>
                    ...
                    <input type="date" name="date_written" value="{{ old('date_written', date('Y-m-d')) }}" required
                        class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                    @error('date_written')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="writer_rank" value="{{ old('writer_rank') }}"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">مكتب الكاتب</label>
                    <input type="text" name="writer_office" value="{{ old('writer_office') }}"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
                </div>
            </div>
            
            <div class="flex justify-end gap-4 pt-4 border-t border-gray-200 dark:border-slate-700">
                <a href="{{ route('books.index') }}" class="px-6 py-2 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700">
                    إلغاء
                </a>
