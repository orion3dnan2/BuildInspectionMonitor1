...
@section('title', 'إضافة سجل جديد - نظام الرقابة والتفتيش')

@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800">إضافة سجل جديد</h1>
    <p class="text-gray-600">أدخل بيانات السجل الجديد</p>
</div>
    ...
    <form action="{{ route('records.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <input type="text" name="military_id" id="military_id" value="{{ old('military_id') }}" required
            ...
            <div>
                ...
                <select name="action_type" id="action_type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    @foreach($actionTypes as $type)
                        <option value="{{ $type }}" {{ old('action_type') == $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
            </div>
