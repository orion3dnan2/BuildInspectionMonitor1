...
<head>
    ...
    <style>
        * { font-family: 'Tajawal', sans-serif; }
        ...
        .sidebar-scroll { height: calc(100vh - 280px); overflow-y: auto; }
    </style>
    <script>
...
<body class="bg-slate-100 dark:bg-slate-900 text-base transition-colors duration-200">
    <div class="flex min-h-screen">
        @auth
        <aside class="w-72 bg-white dark:bg-slate-800 border-l border-slate-200 dark:border-slate-700 fixed h-full flex flex-col shadow-sm">
            <div class="p-5 border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
            ...
            <nav class="flex-1 overflow-y-auto p-4">
                <div class="space-y-1">
                    ...
                    <a href="{{ route('settings.index') }}" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 {{ request()->routeIs('settings.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex-shrink-0 p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                ...
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-center gap-2 px-4 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 rounded-lg transition text-sm">
