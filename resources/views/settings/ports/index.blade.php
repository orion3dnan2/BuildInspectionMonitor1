...
<div class="max-w-6xl mx-auto">
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-2">
            ...
            <span class="text-gray-400">/</span>
            <span class="text-gray-600">المنافذ</span>
        </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-gray-100">
                            <th class="text-right py-3 px-4 text-sm font-medium text-gray-500">اسم المنفذ</th>
                    ...
                    <tbody class="divide-y divide-gray-50" id="portsTable">
                        @forelse($ports as $port)
                        <tr class="hover:bg-gray-50 port-row" data-name="{{ strtolower($port->name) }}">
                            <td class="py-4 px-4 text-sm font-medium text-gray-800">{{ $port->name }}</td>
                            <td class="py-4 px-4 text-sm text-gray-600">{{ $port->type ?? '-' }}</td>
                            <td class="py-4 px-4">
                                <div class="flex items-center gap-2">
                                    <button onclick="editPort({{ $port->id }}, '{{ $port->name }}', '{{ $port->type }}')" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg">
...
<div id="modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-8 w-full max-w-md mx-4">
        <h3 id="modal-title" class="text-xl font-bold text-gray-800 mb-6">إضافة منفذ</h3>
        <form id="port-form" method="POST">
            ...
            <div class="mb-6">
                ...
                <select name="type" id="port-type" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white">
