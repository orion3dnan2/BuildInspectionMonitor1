    ...
    <form action="{{ route('admin.correspondences.update', $correspondence) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-700 dark:text-white focus:ring-2 focus:ring-sky-500 focus:border-transparent">{{ old('description', $correspondence->description) }}</textarea>
                @error('description')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
